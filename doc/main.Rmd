...
# Step0: Load library and data set

```{r step0}
############################ Package installation and loading ##################################

packages.used = c("lsa", "matrixStats", "dplyr", "abind")
...
# Step1: Data preparation and constants setting
...
# Memory-Based Algorithm Step2: Calculate similarity weights
...
# Memory-Based Algorithm Step3: Adding significance weights and variance weights
...
```{r step3, message=FALSE, warning=FALSE, include=FALSE}
#################################################################
...
# Memory-Based Algorithm Step4: Calculating predictions
...
if(!load){
  ...
  MS_pred_sim <- pred_matrix(MS_UI, MS_sim)
  save(MS_pred_sim, file = "../output/MS_pred_sim.RData")
  
  # Calculate predictions for movies based on simrank similarity
  # This calculation took me 1+ hour
  
  movie_pred_sim <- pred_matrix(movie_UI, movie_sim)
  save(movie_pred_sim, file = "../output/movie_pred_sim.RData")
}else{
  ...
  load("../output/movie_pred_sim.RData")

}
```

# Memory-Based Algorithm Step4: Evaluating predictions
...
# Model-based Algorithm Step 2: Calculating parameters and assignment matrix 
...
em_movie <- function(data = movie_UI, C = 5, tau = 0.5, iter = 100)
```

# Model-based Algorithm Step 3: Making predictions

After the E and M steps are complete, to make predictions we take the gamma array, which gives us the probability of a particular
movie being rated a particular score, given a specific cluster, and the assignment matrix which gives us the probability
of a particular user being in any given cluster. We then find the index of the max value of each user to give us the cluster
assignment of that user, and then find the index of the max probability of scores 1 - 6 (or the probabilities in the case of 
the MS data) for each movie (website), given the cluster assignment. With that knowledge we generate a prediction matrix 
similar to our starting matrix of users by movies, with each data point being the predicted score for that user and movie. 
...
movie_pred <- function(gamma = gamma, assign_mat = assign_mat)
ms_pred <- function(gamma = gamma, assign_mat = assign_mat)
